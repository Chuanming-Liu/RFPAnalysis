   ...
   if ~isempty(ttcurves.ttc(indy).d)
      ...
      switch orientation
          case {'yaxis'}
              ...
              hold on;
              handles(indy)=plot(ttcurves.ttc(indy).d,ttcurves.ttc(indy).t,linespec,'MarkerSize',2);
          ...
          case {'xaxis'}
              ...
              hold on;
              handles(indy)=plot(ttcurves.ttc(indy).t,ttcurves.ttc(indy).d,linespec,'MarkerSize',2);
              set(handles(indy),'Tag',ttcurves.ttc(indy).p);
              firstindy=find((~isnan(ttcurves.ttc(indy).d))&(~isnan(ttcurves.ttc(indy).t)));
              if strcmp(dolabel,'dolabel')
                  %%% write phase name label
